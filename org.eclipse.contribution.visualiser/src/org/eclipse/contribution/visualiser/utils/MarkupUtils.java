...
public class MarkupUtils {
   ...
   * @param stripes
   */
  public static void processStripes(List<Stripe> stripes) {
    if (stripes == null)
      return;
    ...
    while (splits > 0) {
      ...
      for (int j = 0; j < stripes.size(); j++) {
        for (int i = 0; i < stripes.size(); i++) {
          if (i != j) {
            Stripe stripe1 = stripes.get(i);
            Stripe stripe2 = stripes.get(j);
            if (debug)
              VisualiserPlugin.log(2, "Processing " + stripe1 + " and " + stripe2); //$NON-NLS-1$ //$NON-NLS-2$
            if (
              stripe2.getOffset() < stripe1.getOffset() ||
              stripe2.getOffset() == stripe1.getOffset() && stripe2.getDepth() < stripe1.getDepth()
            )
            {
              // Changeround
              if (debug)
                VisualiserPlugin.log(2, "Stripe change round"); //$NON-NLS-1$
              stripe2 = stripes.get(i);
              stripe1 = stripes.get(j);
            }
            ...
            if (stripe1end > stripe2start && stripe2end < stripe1end) {
              ...
              if (stripe1end != stripe2end) {
                if (stripe1start != stripe2start) {
                  if (debug) {
                    VisualiserPlugin.log(2, "Stripe split - type 2: stripe1 finishes after stripe2 starts"); //$NON-NLS-1$
                    VisualiserPlugin.log(2, "Stripe1: " + stripe1); //$NON-NLS-1$
                    VisualiserPlugin.log(2, "Stripe2: " + stripe2); //$NON-NLS-1$
                    VisualiserPlugin.log(2, ""); //$NON-NLS-1$
                  }
                  ...
                  if (debug) {
                    VisualiserPlugin.log(2, "Stripe split - type 4: stripe1 and stripe2 start at the same point"); //$NON-NLS-1$
                    VisualiserPlugin.log(2, "Stripe1: " + stripe1); //$NON-NLS-1$
                    VisualiserPlugin.log(2, "Stripe2: " + stripe2); //$NON-NLS-1$
                    VisualiserPlugin.log(2, ""); //$NON-NLS-1$
                  }
                ...
                if (debug) {
                  VisualiserPlugin.log(2, "Stripe split - type 3: stripe2 starts half way down stripe 1 - they both finish at the same point"); //$NON-NLS-1$
                  VisualiserPlugin.log(2, "Stripe1: " + stripe1); //$NON-NLS-1$
                  VisualiserPlugin.log(2, "Stripe2: " + stripe2); //$NON-NLS-1$
                  VisualiserPlugin.log(2, ""); //$NON-NLS-1$
                }
