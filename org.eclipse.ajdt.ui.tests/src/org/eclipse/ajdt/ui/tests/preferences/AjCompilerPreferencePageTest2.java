...
public class AjCompilerPreferencePageTest2 extends UITestCase {
	 ...
	 * 
	 * 1. Press "Apply" and "OK" to rebuild now ==> expect build
	 * 2. Press "OK" and "OK" to rebuild now ==> don't expect build 
	 */
	public void testPerformApplyAndBuild() throws Exception {
		...
		assertFalse("did not expect a build to occur after performOk because " +
				" already applied changes but build happened anyway",didBuild);	
	}
	...
	/**
	 * Change to using project settings
	 * 1. Press "Apply" and "OK" to rebuild now ==> expect build
	 * 2. Press "OK" and "OK" to rebuild now ==> don't expect build 
	 */
	...
	/**
	 * Change back to using workspace settings
	 * 1. Press "Apply" and "OK" to rebuild now ==> expect build
	 * 2. Press "OK" and "OK" to rebuild now ==> don't expect build 
	 */
	...
	/**
	 * Change the non standard options
	 * 1. press "Apply" and "no" to rebuild now ==> don't expect a build
	 * 2. press "Ok" and "no" to rebuild now ==> don't expect a build
	 */
	...
	/**
	 * Change the non standard options
	 * 1. press "Apply" and "yes" to rebuild now ==> expect a build
	 * 2. press "Ok" and "yes" to rebuild now ==> don't expect a build
	 */
