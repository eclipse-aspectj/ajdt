...
public interface ISearchProvider {
    ...
    IJavaElement findITDGetter(IField field);

    IJavaElement findITDSetter(IField field);
    /**
     * Converts the element into something that should be searched for.
     * Should be a no-op if there is no change required
     * @return
     */
    IJavaElement convertJavaElement(IJavaElement origElement);
     ...
     */
    List<SearchMatch> findExtraMatches(PossibleMatch match, SearchPattern pattern, HierarchyResolver resolver) throws JavaModelException;

    /**
     * Callback for after the match has been added to the requestor
     * @param match
     */
    void matchProcessed(PossibleMatch match);
     ...
     */
    IJavaElement filterJUnit4TestMatch(IJavaElement possibleTest) throws JavaModelException;

    boolean isInteresting(IOpenable elt);

    char[] findSource(IOpenable elt);
}
