...
public class UserAopFileExculdeAspectTest extends VisualTestCase {
	...
	public void testUserAopXmlFile() throws Exception {
		...
		assertFalse("aspect was not excluded.", output
				.indexOf("Before advise called") != -1);

	}

	protected void setUp() throws Exception {
		super.setUp();
		IProject project = createPredefinedProject("user.aop.xml.noweave"); //$NON-NLS-1$
		assertTrue(
				"The user.aop.xml.weave project should have been created", project != null); //$NON-NLS-1$
		IProject project2 = createPredefinedProject("WeaveMe3"); //$NON-NLS-1$		
		assertTrue(
