...
import org.aspectj.org.eclipse.jdt.internal.core.dom.rewrite.TokenScanner;
...
public final class AjASTRewriteAnalyzer extends AjASTVisitor {
	...
	private static final String AJ_ASTREWRITEANALYZER_FACTORY = "org.eclipse.ajdt.core.dom.rewrite.AjASTRewriteAnalyzerFactory"; //$NON-NLS-1$
	...
	private final Map<Comparable, TextEdit> sourceCopyInfoToEdit;
  ...
  SourceRange getExtendedRange(ASTNode node) {
	...
	private int rewriteJavadoc(ASTNode node, StructuralPropertyDescriptor property) {
		...
		int pos= rewriteNode(node, property, node.getStartPosition(), AjASTRewriteFormatter.NONE);
		// AspectJ Change end
	...
	class ParagraphListRewriter extends ListRewriter {
      ...
      for (RewriteEvent rewriteEvent : this.list) {
        ASTNode elem = (ASTNode) rewriteEvent.getOriginalValue();
        if (elem != null) {
          if (last != null) {
		...
		if (event != null && event.getChangeKind() != RewriteEvent.UNCHANGED) {
			...
			try {
				...
				if (isAllRemoved) { // all removed: remove right and space up to next element
					...
					String replacement= needsSpaceOnRemoveAll ? String.valueOf(' ') : "";
	...
	private int getPosAfterLeftBrace(int pos) {
		try {
			...
			if (nextToken == TerminalToken.TokenNameLBRACE) {
	...
	public boolean visit(MethodDeclaration node) {
		...
		try {
			...
			if ((extraDims > 0) && (hasExceptionChanges || bodyChangeKind == RewriteEvent.INSERTED || bodyChangeKind == RewriteEvent.REMOVED)) {
				int dim= (Integer) getOriginalValue(node, MethodDeclaration.EXTRA_DIMENSIONS_PROPERTY);
				while (dim > 0) {
	...
	public boolean visit(ReturnStatement node) {
		...
		try {
			int offset= getScanner().getTokenEndOffset(TerminalToken.TokenNamereturn, node.getStartPosition());
	...
	public boolean visit(ArrayCreation node) {
		...
		try {
			int offset= getScanner().getTokenStartOffset(TerminalToken.TokenNameLBRACKET, arrayType.getStartPosition());
        ...
        for (RewriteEvent event : events) {
          ...
          if (changeKind == RewriteEvent.INSERTED) { // insert new dimension
            ...
            int endPos = getScanner().getTokenEndOffset(TerminalToken.TokenNameRBRACKET, elemEnd);
			...
			if (nOldBrackets != nNewBrackets) {
				if (!hasDimensionChanges) {
					offset= getScanner().getTokenEndOffset(TerminalToken.TokenNameRBRACKET, offset);
				}
	...
	public boolean visit(AssertStatement node) {
		...
		rewriteNode(node, AssertStatement.MESSAGE_PROPERTY, offset, AjASTRewriteFormatter.ASSERT_COMMENT);
		// AspectJ Change End
		return false;
	}
	...
	public boolean visit(ForStatement node) {
		...
		try {
			...
			pos= rewriteNode(node, ForStatement.EXPRESSION_PROPERTY, pos, AjASTRewriteFormatter.NONE);
			// AspectJ Change End
	...
	public boolean visit(IfStatement node) {
		...
		if (thenEvent != null && thenEvent.getChangeKind() != RewriteEvent.UNCHANGED) {
			try {
				pos= getScanner().getTokenEndOffset(TerminalToken.TokenNameRPAREN, pos); // after the closing parent
