...
import org.eclipse.jdt.internal.compiler.parser.SourceConstructorDeclaration;
import org.eclipse.jdt.internal.compiler.parser.SourceFieldDeclaration;
import org.eclipse.jdt.internal.compiler.parser.SourceMethodDeclaration;
...
public class AJSourceElementParser2 extends CommentRecorderParser {
	...
	AJCompilationUnitStructureRequestor requestor;
	int fieldCount;
...
public void notifySourceElementRequestor(AbstractMethodDeclaration methodDeclaration) {
	...
	org.aspectj.org.eclipse.jdt.internal.compiler.ast.MethodDeclaration ajtypeMethodDeclaration = new org.aspectj.org.eclipse.jdt.internal.compiler.ast.MethodDeclaration(new org.aspectj.org.eclipse.jdt.internal.compiler.CompilationResult(methodDeclaration.compilationResult.fileName, methodDeclaration.compilationResult.unitIndex, methodDeclaration.compilationResult.totalUnitsKnown,500));
	
	if (ajtypeMethodDeclaration instanceof PointcutDeclaration) {
		selectorSourceEnd = methodDeclaration.sourceStart + methodDeclaration.selector.length - 1;
	}
	if (ajtypeMethodDeclaration instanceof AdviceDeclaration) {
		selectorSourceEnd = methodDeclaration.sourceStart + ((AdviceDeclaration) ajtypeMethodDeclaration).kind.getName().length() - 1; 
	}
	if (isInRange) {
