...
public class AspectJPreferences {
	...
	public static void setProjectAspectPath(IProject project, String path,
		...
		tok = new StringTokenizer(cKinds,
                File.pathSeparator);
		index = 1;
		while (tok.hasMoreTokens()) {
			projectNode.put(ASPECTPATH_CON_KINDS + index, fromContentKind(tok.nextToken()));
			index++;
		}
		...
		index = 1;
		while (tok.hasMoreTokens()) {
			projectNode.put(ASPECTPATH_ENT_KINDS + index, fromEntryKind(tok.nextToken()));
			index++;
		}
	...
	public static void setProjectInPath(IProject project, String path,
		...
		index = 1;
		while (tok.hasMoreTokens()) {
			projectNode.put(INPATH_CON_KINDS + index, fromContentKind(tok.nextToken()));
			index++;
		}
		...
		index = 1;
		while (tok.hasMoreTokens()) {
			projectNode.put(INPATH_ENT_KINDS + index, fromEntryKind(tok.nextToken()));
			index++;
		}
