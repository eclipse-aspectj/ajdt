 ...
 * @author gharley
 */
public class AspectPathLibrariesWorkbookPage extends
    ...
    public AspectPathLibrariesWorkbookPage(IWorkspaceRoot root, ListDialogField aspectpathList) {
        fAspectPathList= aspectpathList;
        fWorkspaceRoot= root;
        fSWTControl= null;
    ...
    private void editAttributeEntry(CPListElementAttribute elem) {
        ...
        if (key.equals(CPListElement.SOURCEATTACHMENT)) {
            ...
            if (result != null) {
                ...
                fLibrariesList.refresh();
                fAspectPathList.refresh(); // images
            }
    ...
    private IClasspathEntry[] getRawClasspath() {
        IClasspathEntry[] currEntries= new IClasspathEntry[fAspectPathList.getSize()];
	...
	public void setSelection(List selection, boolean expand) {
		fLibrariesList.selectElements(new StructuredSelection(selection));
		if (expand) {
			for (int i= 0; i < selection.size(); i++) {
				fLibrariesList.expandElement(selection.get(i), 1);
			}
		}		
	}   
