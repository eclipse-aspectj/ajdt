	...
	static protected String computeTypeName(ASTNode node) {
		...
		while (!(node instanceof org.eclipse.jdt.core.dom.CompilationUnit)) {
			if (node instanceof org.eclipse.jdt.core.dom.AbstractTypeDeclaration) {
				...
				if (typeName == null) {
					typeName = new StringBuilder(identifier);
				} else {
					typeName.insert(0, identifier + "$"); //$NON-NLS-1$
				}
			} else {
				typeName = null;
			}
		...
		StringBuilder packageIdentifier= new StringBuilder(); //$NON-NLS-1$
		if (packageDecl != null) {
			...
			while (packageName.isQualifiedName()) {
				...
				packageIdentifier.insert(0, qualifiedName.getName().getIdentifier() + "."); //$NON-NLS-1$
				packageName= qualifiedName.getQualifier();
			}
			packageIdentifier.insert(0, ((org.eclipse.jdt.core.dom.SimpleName) packageName).getIdentifier() + "."); //$NON-NLS-1$
		}
		return packageIdentifier + typeName.toString();
	}
