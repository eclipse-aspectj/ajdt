...
public abstract class NewTypeWizardPage extends NewContainerWizardPage {
    ...
    implements IStringButtonAdapter, IDialogFieldListener, IListAdapter<InterfaceWrapper>, SelectionListener
  {
    ...
    for (IProblem curr : problems) {
      if (curr.getSourceEnd() < importsEnd) {
        ...
        if (id == IProblem.UnusedImport || id == IProblem.NotVisibleType) { // not visible problems hide unused -> remove both
          ...
          for (Object importsDecl : importsDecls) {
            ImportDeclaration decl = (ImportDeclaration) importsDecl;
    ...
    //$NON-NLS-1$
    String buf = "public class " + getTypeName() +
                 "{ }"; //$NON-NLS-1$
    return buf;
	}
	...
	protected String getTemplate(String name, ICompilationUnit parentCU, int pos) {
		...
		} catch (CoreException | TemplateException | BadLocationException e) {
			JavaPlugin.log(e);
		}
    return null;
	}
    ...
    for (IMethod method : typeMethods)
      handleIds.add(method.getHandleIdentifier());
