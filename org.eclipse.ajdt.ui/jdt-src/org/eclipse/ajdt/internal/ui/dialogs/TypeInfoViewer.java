...
public class TypeInfoViewer {
	...
	private static class SearchRequestor extends TypeNameMatchRequestor {
		...
		public TypeNameMatch[] getResult() {
			return fResult.toArray(new TypeNameMatch[0]);
		}
	...
	protected static class TypeInfoComparator implements Comparator<TypeNameMatch> {
		...
		@Override
		public int compare(TypeNameMatch left, TypeNameMatch right) {
			int leftCategory = getCamelCaseCategory(left);
			int rightCategory = getCamelCaseCategory(right);
			if (leftCategory < rightCategory)
				return -1;
			if (leftCategory > rightCategory)
				return +1;
			int result = compareName(left.getSimpleTypeName(), right.getSimpleTypeName());
			if (result != 0)
				return result;
			result = compareTypeContainerName(left.getTypeContainerName(), right.getTypeContainerName());
			if (result != 0)
				return result;

			leftCategory = getElementTypeCategory(left);
			rightCategory = getElementTypeCategory(right);
			if (leftCategory < rightCategory)
				return -1;
			if (leftCategory > rightCategory)
				return +1;
	...
	protected static class TypeInfoLabelProvider {
		...
		public TypeInfoLabelProvider(ITypeInfoImageProvider extension) {
			...
			for (IVMInstallType install : installs)
				processVMInstallType(install, locations, labels);
			fInstallLocations = locations.toArray(new String[0]);
			fVMNames = labels.toArray(new String[0]);
	...
	private static class ProgressUpdateJob extends UIJob {
		private final TypeInfoViewer fViewer;
		...
		private String getMessage() {
			if (fDone) {
				...
				return Messages.format(
					...
					new Object[] { fName, (int) ((fWorked * 100) / fTotalWork) });
			}
		}
	}
	...
	private static abstract class AbstractSearchJob extends AbstractJob {
		...
		protected abstract TypeNameMatch[] getSearchResult(Set<TypeNameMatch> matchIdsInHistory, ProgressMonitor monitor)
		...
		private void internalRunVirtual(ProgressMonitor monitor) throws CoreException {
			if (monitor.isCanceled())
				throw new OperationCanceledException();
			...
			Collections.addAll(filteredMatches, matchingTypes);
	...
	private static class CachedResultJob extends AbstractSearchJob {
		...
		protected TypeNameMatch[] getSearchResult(Set<TypeNameMatch> filteredHistory, ProgressMonitor monitor) {
			...
			TypeNameMatch[] types = result.toArray(new TypeNameMatch[0]);
			if (fFilter.isCamelCasePattern())
	...
	private static class ImageManager {
		private final Map fImages = new HashMap(20);
	...
	public void reset() {
		...
		if (VIRTUAL) {
			...
			for (int i = 0; i < historyItems.length; i++) {
				...
				addSingleElement(type, fLabelProvider.getImageDescriptor(type), fLabelProvider.getText(last, type, next));
				last = type;
				type = next;
			}
			shortenTable();
	...
	private void addHistory(
		int ticket,
		final List<TypeNameMatch> elements,
	...
	private void addSingleElement(TypeNameMatch element, ImageDescriptor imageDescriptor, String label) {
		TableItem item;
		Object old = null;
		if (fItems.size() > fNextElement) {
			item = fItems.get(fNextElement);
	...
	private void shortenTable() {
		...
		if (fItems.size() > fNextElement)
			fItems.subList(fNextElement, fItems.size()).clear();
	}
