...
public class PointcutRenameParticipantTest extends AJDTCoreTestCase {
	...
	public void testTJPTypeRename() throws Exception {
		...
		try {
			...
			assertNotNull("getModifiedElement should not be null", change //$NON-NLS-1$
					.getModifiedElement());
			assertTrue("Modified element should be GetInfo.aj", //$NON-NLS-1$
					change.getModifiedElement().toString()
							.indexOf("GetInfo.aj") != -1); //$NON-NLS-1$
	...
	public void testBeanTypeRename() throws Exception {
		...
		try {
			...
			assertNotNull("getModifiedElement should not be null", change //$NON-NLS-1$
					.getModifiedElement());
			assertTrue("Modified element should be BoundPoint.aj", //$NON-NLS-1$
					change.getModifiedElement().toString()
							.indexOf("BoundPoint.aj") != -1); //$NON-NLS-1$
