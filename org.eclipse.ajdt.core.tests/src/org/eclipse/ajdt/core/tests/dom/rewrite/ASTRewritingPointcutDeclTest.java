...
public class ASTRewritingPointcutDeclTest extends AJDTCoreTestCase {
	...
	public void testAddImportStatement() throws Exception {
		...
		try {
			...
			AjASTRewrite rewriter = AjASTRewrite.create(ast); // the class I'm testing
			AjListRewrite lrw = rewriter.getListRewrite(cu, // another class I'm testing
	...
	public void testMethodDeclChanges() throws Exception {
		...
		try {
			...
			{ // change return type
				...
				assertTrue("Has no return type: gee", methodDecl.getReturnType() != null); //$NON-NLS-1$
			...
			{ // remove return type
				...
				assertTrue("Has no return type: hee", methodDecl.getReturnType() != null); //$NON-NLS-1$
			...
			{ // rename first param & last throw statement
				...
				assertTrue("must be 2 thrown exceptions", thrownExceptions.size() == 2); //$NON-NLS-1$
			...
			{ // rename first and second param & rename first and last
				...
				List thrownExceptions = methodDecl.thrownExceptions();
				assertTrue("must be 3 thrown exceptions", thrownExceptions.size() == 3); //$NON-NLS-1$
				Name newThrownException1 = ast.newSimpleName("ArrayStoreException"); //$NON-NLS-1$
				Name newThrownException2 = ast.newSimpleName("InterruptedException"); //$NON-NLS-1$
